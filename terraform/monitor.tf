resource "openstack_networking_floatingip_v2" "monitor_floating_ip" {
  pool       = var.public
  depends_on = [openstack_networking_router_interface_v2.router_interface]
}

resource "openstack_networking_port_v2" "monitor_port_management" {
...
resource "openstack_networking_floatingip_associate_v2" "monitor_floating_ip_association" {
  floating_ip = openstack_networking_floatingip_v2.monitor_floating_ip.address
  port_id     = openstack_networking_port_v2.monitor_port_management.id
}

resource "openstack_blockstorage_volume_v3" "monitor_base_volume" {
  count             = 0
  image_id          = data.openstack_images_image_v2.image.id
  name              = "${var.prefix}-volume-monitor-base"
...
resource "openstack_compute_instance_v2" "monitor_server" {
  ...
  network { port = openstack_networking_port_v2.monitor_port_management.id }
...
write_files:
  - content: |
      ...
      PORTS = {
          "${openstack_networking_port_v2.monitor_port_management.mac_address}": [
              "192.168.64.5/20",
              "192.168.96.5/20"
          ]
      }
  ...
  - content: |
    ...
    permissions: '0700'
  - content: |
      ${indent(6, file("files/monitor-part-1.yml"))}
    path: /opt/monitor-part-1.yml
    permissions: '0644'
  - content: |
      ${indent(6, file("files/monitor-part-2.yml"))}
    path: /opt/monitor-part-2.yml
    permissions: '0644'
  - content: |
      ${indent(6, file("files/monitor-part-3.yml"))}
    path: /opt/monitor-part-3.yml
    permissions: '0644'
  - content: |
  ...
  - content: |
      ...
      sudo -iu dragon ansible-playbook -i testbed-monitor.osism.test, /opt/monitor-part-1.yml -e configuration_git_version=${var.configuration_version}
      ...
      sudo -iu dragon ansible-playbook -i testbed-monitor.osism.test, /opt/monitor-part-2.yml
      sudo -iu dragon ansible-playbook -i testbed-monitor.osism.test, /opt/monitor-part-3.yml

      sudo -iu dragon cp /home/dragon/.ssh/id_rsa.pub /opt/ansible/secrets/id_rsa.operator.pub

      # NOTE(berendt): wait for ARA
      until [[ "$(/usr/bin/docker inspect -f '{{.State.Health.Status}}' monitor_ara-server_1)" == "healthy" ]]; do
          sleep 1;
      done;
      ...
      # NOTE: Restart the monitor services to update the /etc/hosts file
      sudo -iu dragon sh -c 'docker-compose -f /opt/monitor/docker-compose.yml restart'

      # NOTE(berendt): wait for ARA
      until [[ "$(/usr/bin/docker inspect -f '{{.State.Health.Status}}' monitor_ara-server_1)" == "healthy" ]];
      do
          sleep 1;
      done;
